...
class MySystemWrapperClass : public MySystemWrapperInterface {
  public:
    FILE* fopen(const char* filename, const char* mode) { return ::fopen(filename, mode); }

    int fseek(FILE* stream, long int offset, int whence) { return ::fseek(stream, offset, whence); }

    size_t fread(void* ptr, size_t size, size_t nmemb, FILE* stream) { return ::fread(ptr, size, nmemb, stream); }
...
class MyClass {
  ...
  public:
    ...
    MyClass(MySystemWrapperInterface* system_wrapper) : system_wrapper_(system_wrapper) {}
...
TEST(test, Test1fseek) {
    ...
    EXPECT_CALL(mock_system_wrapper, fopen).Times(1).WillOnce(Return(reinterpret_cast<FILE*>(0x1)));
